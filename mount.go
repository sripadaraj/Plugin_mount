...
func (m *mountManager) Create(name, hostdir string, opts map[string]string) *mount1 {
	...
	if found && c.connections > 0 {
		...
		mnt := &mount1{name: name, hostdir: hostdir, managed: true, opts: opts, connections: 0}
		m.mounts[name] = mnt
		return mnt
	}
}
...
func (m *mountManager) GetVolumes(rootPath string) []*volume.Volume {
	...
	for _, mount1 := range m.mounts {
		volumes = append(volumes, &volume.Volume{Name: mount1.name, Mountpoint: mount1.hostdir})
	}
	return volumes
}
